...
import { detectSpecialType } from '@/util/clipboard-parser';
...
const LinkPreview: React.FC<LinkPreviewProps> = ({ entry }) => {
  ...
  const previewCheck = detectSpecialType(entry.preview || '');
  const contentCheck = detectSpecialType(entry.content || '');
  const url =
    previewCheck.type === 'link'
      ? previewCheck.value
  ...
  return (
    ...
    >
      <Card className='overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-[1.01] border-border/50 hover:border-border group-hover:bg-muted/30 h-full flex flex-col'>
        {data.image_url && (
        ...
        <div className='p-4 space-y-3 flex-shrink-0'>
          <div className='space-y-2'>
            <CardTitle className='text-base font-semibold line-clamp-2 leading-tight text-balance group-hover:text-primary transition-colors duration-200'>
              {data.title}
            </CardTitle>
